...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if let config = ConfigurationOptions.load() {
            ZscalerSDK.sharedInstance().configuration = config.zscalerConfiguration
        } else {
            logger.error("Could not load saved configuration, using default")
            ZscalerSDK.sharedInstance().configuration = ZscalerSDKConfiguration(
